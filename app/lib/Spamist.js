...
Spamist.prototype.readtext = function (text) {
			...
			var j_ham =1
			for(var i in jsonData_ham){
				var key_ham = i
				var val_ham = jsonData_ham[i]
				contents_ham[j_ham]=val_ham['content']
				contents_ham[j_ham] = contents_ham[j_ham].replace(/#|_|\ـ|-|'|]|\[|\*|\+|\,|\!|\&|\%|\$|\#|\?|\.|\'|\/|\@|\(|\)|\^/g,'');
				contents_ham[j_ham] = contents_ham[j_ham].replace(/#| a | an | and | or /g,' ');
				contents_ham[j_ham] = contents_ham[j_ham].toLowerCase()
				contents_ham[j_ham] = "ham	" + contents_ham[j_ham]
