...
function post_similarity($newpost , $oldpost)
{
    $curl = curl_init();
    $PostBody[0]=$newpost;
    $PostBody[1]=$oldpost;
    ...
    curl_setopt_array($curl, array(
        ...
        CURLOPT_ENCODING => "",
        CURLOPT_CUSTOMREQUEST => "POST",
        CURLOPT_POSTFIELDS => "{\"jsonrpc\": \"2.0\", \"method\": \"similarity\", \"params\": [\"" . $PostBody[0] . "\" , \"" .$PostBody[1]."\"] , \"id\":1}",
        CURLOPT_HTTPHEADER => array(
            "Authorization: Basic " . base64_encode($credentials),
            "cache-control: no-cache",
            "content-type: application/json"
...
//$credentials = "یکی نیاد پیوی من";
//$postbodyCURL="یکی بیاد پیوی من";
//$response1=post_similarity($postbodyCURL , $credentials);
//echo $response1;
